...
(function ($) {
	...
	$.fn.interactiveMap = function (options) {
		...
		var _createObject = function (name, coords, data, actions) {
			...
			//Если необходимо выделить активный элемент при загрузке, то скрываем его фон и устанавливаем флаг
			// активности
		...
		var _showByOne = function (id, time, count) {
			...
			setTimeout(function () {
				var search = $("." + id);
				var deferred = search.fadeIn(time, function () {
					self = search;
				}).promise();
		...
		var _searchAndRenderObj = function (objId, type) {
			var curItem = $.grep(_itemsScope, function (e) {
				return e.id == objId;
			});
			if (curItem.length == 1) {
		...
		var methods = {
			...
			createItems: function () {
				...
				var _img = _checkImg();
				//Если изображение есть, то ждем пока оно загрузится, в противном случае, грузим сгенерированные DIV
				// сразу
				if (_img) {
